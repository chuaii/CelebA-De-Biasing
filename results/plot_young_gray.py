...
CSV_PATH = Path(__file__).parent / "Young_GrayHair.csv"
OUT_PATH = Path(__file__).parent / "young_gray_comparison.png"
...
GROUP_COLS = [
    "acc_Young_NonGray",
    "acc_Young_Gray",
    "acc_NonYoung_NonGray",
    "acc_NonYoung_Gray",
]
GROUP_LABELS = ["Y / NG", "Y / G", "NY / NG", "NY / G"]
...
def plot_bias(ax: plt.Axes, df: pd.DataFrame) -> None:
    ...
    ax.set_title("(a) Bias in Baseline\n(Y = Young, NY = Non-Young, G = Gray Hair, NG = Non-Gray)",
...
def main() -> None:
    ...
    fig.suptitle("Young × Gray Hair — Bias & De-biasing Comparison",
